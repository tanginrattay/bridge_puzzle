...
void back_tracking_helper(int n,int m,int** c,int c_size,int index, int* trigger);

int back_tracking(int n,int m,int** c,int c_size){
    int trigger = 0;
    back_tracking_helper(n,m,c,c_size,0, &trigger);
    return trigger;
};
void back_tracking_helper(int n,int m,int** c,int c_size,int index, int* trigger){
     //终点，num==c_size
     ...
     int y = c[index][1];
     if(map[x][y][5] == 0){
        back_tracking_helper(n, m, c, c_size, index + 1, trigger);
        return;
     }
     //若存在接口，则进行试探
     else{
