...
.col {
  flex-basis: 48%; /* 每个栏目占据 48% 的宽度，中间留出空隙 */
  /* 你也可以使用 flex: 1; 来平分宽度 */
}
</style>
...
当$D_{i,j} > 0$时，该点为一个连接点，该数字表示该连接点需要连出的线段数量（即**度数**或**接口数**）。
--v--
**目标：** 找到一种连线方式，使得：
* 所有连接点的连线数量（实际度数）满足其给定的数字 $D_{i,j}$。
* 任意两条连线不能交叉。
* 连线只能沿着网格线进行。
...
### 1. 数据结构
--v--
**数据结构：** 使用三维数组 `map[51][51][6]` 存储信息（基于 $n, m \le 50$）：
...
* **接口数量剪枝：**
    * 从左上角按写字顺序开始遍历，每个节点只考虑向右和下连线。
    * 当遍历到某个节点，而该节点的剩余接口数量大于2时，return。
...
```
* 用于检查从 $(i, j)$ 向给定方向 `dir` 延伸，是否能找到一个**可连接**（`map[i][j][0]>0` 且 `map[i][j][5]>0`）的节点。
...
* **递归逻辑：** 
    * 根据当前节点 $(x, y)$ 的**剩余接口数** (`rem = map[x][y][5]`)，确定需要尝试的连线组合。
...
--v--
+ **测试难点**：当测试输入的大小相同时，测试输入中点的*密度*、*分布*、*值*会显著影响运行时间。因此，如果采用随机生成测试输入的方法，无法反映运行时间和输入规模的关系。
+ **解决方法**：我们先取一个$5 \times 5$的输入样例，然后通过多个$5 \times 5$块拼接形成$10 \times 10$、$15 \times 15$等规模的输入，由此保证测试输入中点的*密度*、*分布*、*值*大体不变。
--v--
基本的$5 \times 5$块：
```cpp
5 5
...
|输入规模|运行次数|运行时间|
|:---:|---:|:---:|
|$5 \times 5$|10000000|00:00:10.661|
|$10 \times 10$|5000000|00:01:16.790|
|$20 \times 20$|1000000|00:01:19.299|
|$30 \times 30$|1000|00:00:03.696|
|$40 \times 40$|20|00:00:07.308|
|$50 \times 50$|3|00:01:43.326|
...
|输入规模|运行次数|运行时间|
|:---:|---:|:---:|
|$5 \times 5$|10000000|00:00:04.572|
|$10 \times 10$|5000000|00:00:12.235|
|$20 \times 20$|1000000|00:01:07.299|
|$30 \times 30$|1000|00:00:05.701|
|$40 \times 40$|20|00:00:11.182|
|$50 \times 50$|3|00:02:32.987|
...
|输入规模|运行次数|运行时间|
|:---:|---:|:---:|
|$10 \times 20$|1000000|00:00:43.126|
|$20 \times 30$|10000|00:00:35.650|
|$30 \times 40$|100|00:00:57.509|
|$20 \times 80$|10000|00:00:02.903|
...
|输入规模|运行次数|运行时间|
|:---:|---:|:---:|
|$10 \times 20$|1000000|00:00:37.980|
|$20 \times 30$|10000|00:01:01.735|
|$30 \times 40$|100|00:01:24.535|
|$20 \times 80$|20|00:01:49.728|

</div>

--v--
### 分析
+ 当输入规模较小时，backtracking在运行中基本没有怎么回溯，可能只需回溯几次就跑完了整个图。而加上optimize后，optimize阶段会反复遍历整个图，直到找不出优化的点。因此加上optimize反而增加了运行时间。
+ 当输入规模较大时，backtracking需要回溯的层数变多，使用optimize预处理可以减小需要回溯层数。因此使用optimize可以缩短运行时间。
+ 当输入规模为长方形时，$\frac{周长}{面积}$ 更大，意味着optimize优化空间更大，所以此时optimize的优势更加显著。
...
--v--
### 总结
从测试数据得到，Backtracking的时间复杂度为$O(2^{\sqrt(n \times m)})$。而总体的时间复杂度难以分析，因为在实际运行时，optimize受形状的影响很大。
